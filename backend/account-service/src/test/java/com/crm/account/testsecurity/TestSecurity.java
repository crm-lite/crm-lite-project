package com.crm.account.testsecurity;
 ...
 * customer-service). The starter's real decoder (signature/issuer/audience against
 * Keycloak) is overridden here — an override path crm-security-starter explicitly
 * supports — so the FULL security filter chain, role mapping and audit attribution
 * still run against known tokens. Real-cryptography validation is covered by the
 * gateway's Keycloak Testcontainers suite and the starter's validator tests.
 */
