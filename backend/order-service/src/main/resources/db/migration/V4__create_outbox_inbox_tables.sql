-- Reliable-messaging foundation for order_db (ADR-017 §8). Project addition, not a
-- workbook table -- exactly like order_number_seq (V1) and idempotency_key (V3).
--
...
-- crm.messaging.outbox.relay.enabled both ship false (ADR-017 §11) and POST /api/orders
-- is byte-for-byte unchanged. The tables exist so the cutover is a configuration change.
