 ...
 * the product domain's needs: the GNL_ST GENERAL statuses (ACTV/PASV) plus PNDG,
 * which lives in the PROD domain and carries the "reserved by an in-flight sale, not
 * yet committed" meaning the write slice gives it (ADR-015 §4.2/§5.2).
 *
 * Semantics:
 ...
 *
 * <p>Reads are unaffected by all of this (ADR-002 §8): active-record filtering still
 * uses the local {@link LookupContract} constants, so no query makes a remote call
 * per row or per request. Only writes need this service — and only writes fail closed.
 */
@Service
public class LookupCatalogService {
    ...
    private static final Map<String, Long> STATUS_CONTRACT_IDS = Map.of(
            ...
            LookupContract.STATUS_PENDING, LookupContract.STATUS_PENDING_ID);
