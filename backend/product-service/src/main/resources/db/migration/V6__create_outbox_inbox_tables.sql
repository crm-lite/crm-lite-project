-- Reliable-messaging foundation for product_db (ADR-017 §8). Project addition, not a
-- workbook table -- exactly like sale_operation (V5).
--
...
-- crm.messaging.outbox.relay.enabled both ship false (ADR-017 §11), and the synchronous
-- POST /api/products the SALE orchestration calls is unchanged. The tables exist so the
