...
| FR | Capability | Notes |
|---|---|---|
| FR-SALE-01 | Product sale, end to end | **Asynchronous since 10.08.2026 (ADR-018).** `POST /api/orders/drafts` creates the order in `WAIT` with its KR-12 number (so the Submit screen can show the Order ID — analyst decision A1); `POST /api/orders/{n}/submit` writes the items, moves `WAIT → MIDLWARE`, creates the saga and records its first command **in one local transaction**, and answers **202** without waiting for anything downstream; `GET /api/orders/{n}/status` is polled. order-service then orchestrates: check account → products **PNDG** → link to account → activate to `ACTV` → terminal `sale-completed`. Failure before any product exists cancels the order; an involvement failure compensates the products; an activation failure compensates the **involvement first**, then the products. A compensation that itself fails is escalated to an operator, not swallowed. The synchronous `POST /api/orders` (ADR-016 §5) is **deprecated** and retained only for the current frontend and rollback |
| FR-SALE-01 (draft) | AC-SALE-01-12 Order ID on the Submit screen, AC-SALE-01-16 abandoned sale | The analyst clarified (10.08.2026) that the records exist before approval with a status that is *not* MIDLWARE — the workbook's existing `GNL_ST WAIT` is that status, and **no new status row is invented**. A WAIT draft has no items, no products, no saga and no command naming it, so AC-SALE-01-16 still holds **by construction** whether or not the browser calls `DELETE /api/orders/{n}/draft`; a cleanup job cancels expired drafts and can never submit one |
...
| FR-SALE-01 (amounts) | AC-SALE-01-12 Total Amount | **Snapshot** columns `cust_ord.total_amount` / `cust_ord_item.amount` — project additions (ADR-016 §2.4): a past order's total must not move when the price list does. The values feeding them are **non-normative demo fixtures**, confirmed by the analyst on 10.08.2026 as demo data and not a blocker (document-delta A3, superseding the P1/P5 "approval pending" status). No price list is expected and none is invented |
...
| KR-12 | Order Number | `[T][YY][SSSSSS][C]`, Luhn, per-segment/per-year sequence — the KR-11 shape reused. Immutable, never reused, including after a compensated sale. **Settled 10.08.2026:** the analyst required *uniqueness* and left the rule to the technical team, so this format is a **project technical choice satisfying an analyst requirement** — not analyst-mandated, and no longer an open sign-off (ADR-016 §4, ADR-018 §1.2). Allocated at **draft creation**, so the Order Number is displayable before Submit |
