...
| AC-SALE-01-12 "Order ID shown on the Submit screen" | **Resolved by the analyst on 10.08.2026** (ADR-018 §1.1): the records exist before approval with a status that is not MIDLWARE, so the Order ID may be shown at that point. `POST /api/orders/drafts` creates the order in the workbook's existing `GNL_ST WAIT` and allocates the KR-12 number there. **Supersedes ADR-016 §8.3's "after submission" reading**, which had been flagged for analysts | ASYNC-IT `draftCreatesWaitOrderWithNumber` |
| AC-SALE-01-16 an abandoned sale is never processed later | still true **by construction**, and now for a stronger reason than "there is nothing to abandon": the draft creates the ORDER, not the basket, and a WAIT order has no items, no products, no saga and **no command anywhere that names it**, so nothing can process it whether or not the browser calls the abandon endpoint. A cleanup job cancels expired drafts and has no code path that could submit one | ASYNC-IT `abandonedDraftCannotBeSubmitted`, `staleDraftCleanupCancelsOnly`, `draftCreatesWaitOrderWithNumber` |
...
| KR-12 Order Number (**a project format satisfying the analyst's uniqueness requirement**, settled 10.08.2026) | `[T][YY][SSSSSS][C]`, Luhn, `order_number_seq`; immutable, never reused — including after a compensated sale. Allocated at draft creation so it is displayable before Submit | `OrderNumberFormatTest`, `LuhnCheckDigitTest`, ORD-IT `sequenceIsGaplessAcrossCompensations`, ASYNC-IT `draftCreatesWaitOrderWithNumber` |
| Sale orchestration across three databases without a distributed transaction (**deprecated route**) | ADR-016 §5: local order write → products PNDG → attach → confirm → **involvement (commit point)**; every earlier failure compensated | ORD-IT `basketRejectionIsRelayedVerbatim`, `productServiceUnavailableAtCreate`, `confirmFailureCompensates`, `involvementWriteUnavailableCompensates`, `involvementWriteRefusedSurfacesAsConflict` |
| Sale orchestration as a persisted saga (**the live route**, ADR-018) | order-service owns `sale_saga` (`sagaId = orderNumber`); WAIT→MIDLWARE + saga + first command in one commit; 202 without waiting; products stay PNDG until the involvement exists; activation failure compensates involvement then products; a failed compensation escalates to `MANUAL_INTERVENTION` and leaves the order MIDLWARE rather than claiming an undo | ASYNC-IT `submitIsAtomicAndAsynchronous`, `happyPath`, `involvementFailureCompensatesProducts`, `activationFailureCompensatesInvolvementThenProducts`, `failedCompensationEscalates` |
| Correctness under at-least-once delivery | duplicate absorbed by the Inbox; an out-of-order reply is counted and does not advance; optimistic locking settles a race; a reply for an unknown saga is ignored; unsafe failure text never reaches a client; commands reissued idempotently; a terminal saga is never reissued | ASYNC-IT `duplicateReplyIsAbsorbed`, `outOfOrderReplyDoesNotAdvance`, `replyForUnknownSagaIsIgnored`, `unsafeFailureKeyIsReplaced`, `stuckSagaCommandIsReissued`, `exhaustedRetryBudgetEscalates`, `terminalSagaIsNeverReissued`; `SaleCommandHandlerTest` (product + account) |
| Legacy synchronous and asynchronous routes can never both act on one sale | the legacy endpoint creates a NEW order already MIDLWARE; submit accepts only WAIT | ASYNC-IT `legacyAndAsyncCannotBothRunOneSale`, `onlyWaitMaySubmit` |
| GNL_ST: `WAIT` for a pre-submit draft, `MIDLWARE` from Submit, `CANCELLED` system-only | ADR-018 §1.1/§6. `CANCELLED` is reached by compensation, an abandoned draft or an expired one — **never** by a user cancelling a submitted order (KR-7 out of phase); the abandon endpoint refuses a MIDLWARE order | ORD-IT `cancelledOrderKeepsItsNumberAndStaysVisible`; ASYNC-IT `abandonedDraftCannotBeSubmitted`, `onlyWaitMaySubmit` |
| Public `processingStatus` is NOT a GNL_ST value | `DRAFT`/`PROCESSING`/`COMPLETED`/`FAILED` is order-service's own API contract; a completed sale stays `MIDLWARE` because no accepted requirement defines another terminal ORDER status | ASYNC-IT `happyPath` |
| Idempotency foundation for `POST /api/orders` (**project addition** — no FR/AC covers this; engineering follow-up to the ADR-016 §5.3b httpclient5-retry incident, 2026-08-06) | required `Idempotency-Key`; same key+body → replay, different body → 409 conflict, concurrent same key → 409 (DB UNIQUE constraint is the concurrency guard); same key forwarded as product-service's `saleOperationId` (ADR-015 §9) and to sale-scoped `/api/products/compensate` | ORD-IT `duplicateSameKeySameRequestReplaysOriginalResult`, `sameKeyDifferentPayloadConflicts`, `concurrentSameKeyRequestCannotCreateDuplicateOrder`, `missingOrMalformedIdempotencyKeyIsRejected`; PROD-IT `duplicateSaleOperationIdReplaysWithoutDuplicating`, `concurrentSaleOperationInProgressIsRefused`, `compensatePassivatesOnlyPendingProductsAndIsIdempotent`; frontend `order-submit.store.spec.ts` (Idempotency-Key reuse) |
...
| ~~AC-ADDR-04-04 address in-use check, Billing Account branch~~ | — | ✅ **implemented 2026-08-05 (BUG-API-ADDR-04-03)** — `AddressServiceImpl` fetches the customer's accounts via `AccountServiceClient.listAccounts`; an Active account whose `billingAddressId` matches the address blocks delete with 409 `MSG-ADDR-IN-USE`; account-service unavailable fails closed with 503 |
| AC-ADDR-04-04 address in-use check, service-address/product-service branch | customer-service → product-service (**still open**) | documented no-op |
