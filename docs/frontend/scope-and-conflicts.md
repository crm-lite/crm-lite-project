...
| # | Konu | Karar | Durum |
|---|---|---|---|
| 1A.1 | **Hesap bölümü kapsamı** | Customer Info hesap bölümü geliştirilecek kapsama girdi → yeni kardeş feature **`features/account/`** (FE-ADR-003 §Consequences'ın öngördüğü evrim). Kontrat: `/api/accounts/**` — tam 5 endpoint, `docs/api/account-service.md` | 🟢 karara bağlandı |
...
| 2A.5 | **Sonuç satırına tıklama** | Satır tıklanınca Customer Info'ya gidiyor | Detay endpoint'i **var** (`GET /api/customers/{customerNumber}`) | **UYGULANDI (2026-07-25)** öneriye göre: Customer ID hücresi link görünümünde ama **etkileşimsiz** (`aria-disabled`, gezinme yok) — Detail rotası gelince `routerLink`e çevrilir; `data-testid` (`…-open-link`) şimdiden yerinde | 🟢 karara bağlandı (pasif; Detail ile aktifleşir) |
